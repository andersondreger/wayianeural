...
export function Dashboard({ user, onLogout }: DashboardProps) {
  ...
  const handleDelete = async (name: string) => {
    if (!confirm(`Deseja destruir o cluster ${name}?`)) return;
    try {
      const res = await fetch(`${EVOLUTION_URL}/instance/delete/${name}`, { method: 'DELETE', headers: HEADERS });
      if (res.ok) {
    ...
    <div className="flex h-screen bg-[#050505] text-white font-sans overflow-hidden">
      ...
      >
        <div className="mb-12 flex items-center justify-between">
          ...
          <button onClick={() => setIsSidebarExpanded(!isSidebarExpanded)} className="p-2 glass rounded-xl text-orange-500 shrink-0">
        ...
        <nav className="flex-1 space-y-3">
          {[
            ...
            >
              ...
              {isSidebarExpanded && <span className="text-[10px] font-black uppercase tracking-widest truncate">{tab.label}</span>}
            </button>
          ))}
        </nav>
          ...
          <div className="flex-1 p-10 lg:p-20 overflow-y-auto custom-scrollbar">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <GlassCard className="!p-10 !rounded-[3.5rem] border-white/5 bg-black/20">
                <h3 className="text-xl font-black uppercase italic mb-8 flex items-center gap-3"><Plus size={22} className="text-orange-500" /> Provisionar Engine</h3>
                ...
                <div className="space-y-4">
                  {instances.map(inst => (
                    <div key={inst.id} className="p-8 bg-white/[0.01] border border-white/5 rounded-[2.5rem] flex items-center justify-between group hover:border-orange-500/20">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] font-black uppercase text-orange-500 tracking-widest flex items-center gap-2"><Smartphone size={10} /> {inst.phone || 'NÃO VINCULADO'}</div>
                        </div>
                      </div>
                      <div className="flex gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openQR(inst.name)} className="p-4 glass rounded-xl text-orange-500 hover:bg-orange-600 hover:text-white transition-all"><RefreshCw size={18}/></button>
                        <button onClick={() => handleDelete(inst.name)} className="p-4 glass rounded-xl text-red-500 hover:bg-red-600 hover:text-white transition-all"><Trash2 size={18}/></button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex-1 flex flex-col relative z-10">
                  <header className="p-8 border-b border-white/5 flex items-center justify-between bg-black/20 backdrop-blur-xl">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 rounded-3xl bg-orange-600/10 flex items-center justify-center text-orange-500 font-black italic text-2xl border border-orange-500/10 overflow-hidden shadow-2xl">
                        {currentLead.avatar ? (
                          <img 
                            src={currentLead.avatar} 
                            className="w-full h-full object-cover" 
                            alt="" 
                            onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement!.innerHTML = `<span>${currentLead.contactName[0].toUpperCase()}</span>`; }}
                          />
                        ) : (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-black uppercase text-gray-700 tracking-[0.2em]">Engine: {(currentLead as any).instanceSource} • {currentLead.contactPhone}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-2"><GlassButton className="!p-4 hover:text-orange-500"><Smartphone size={18}/></GlassButton><GlassButton className="!p-4 hover:text-orange-500"><MoreVertical size={18}/></GlassButton></div>
                  </header>
                  ...
                  <div className="flex-1 overflow-y-auto p-12 space-y-6 custom-scrollbar scroll-smooth">
                    {chatMessages.map((m, idx) => (
                      <div key={idx} className={`flex ${m.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[70%] p-6 rounded-[2.5rem] shadow-2xl relative ${m.sender === 'me' ? 'bg-orange-600 rounded-br-none text-white' : 'bg-white/[0.04] border border-white/5 rounded-bl-none text-gray-200'}`}>
                          <p className="text-sm font-medium leading-relaxed tracking-tight">{m.text}</p>
                          <div className={`text-[7px] font-black uppercase mt-3 tracking-[0.3em] ${m.sender === 'me' ? 'text-white/40' : 'text-gray-700'}`}>{m.time} • {m.sender === 'me' ? 'SISTEMA' : 'CLIENTE'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="p-10 border-t border-white/5 bg-[#080808]/80 flex gap-4 backdrop-blur-2xl">
                    <button className="p-6 glass rounded-[2rem] text-gray-700 hover:text-orange-500"><Paperclip size={24}/></button>
                    <input 
                      value={messageInput} onChange={e => setMessageInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()}
                      placeholder="ESCREVA SUA RESPOSTA..." className="flex-1 bg-white/[0.01] border border-white/10 rounded-[2.5rem] py-6 px-12 text-xs font-bold uppercase outline-none focus:border-orange-500 transition-all placeholder:text-gray-900"
                    />
                    <button onClick={handleSend} disabled={isSending || !messageInput.trim()} className="p-6 bg-orange-600 rounded-[2.5rem] hover:bg-orange-500 shadow-lg flex items-center justify-center min-w-[90px]">
                      {isSending ? <Loader2 className="animate-spin" size={28} /> : <Send size={28}/>}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-20 opacity-20"><MessageSquare size={60} className="mb-10" /><h3 className="text-4xl font-black uppercase italic tracking-tighter">Terminal CRM</h3><p className="text-[10px] font-black uppercase tracking-[0.6em] mt-4">Aguardando Handshake com Lead</p></div>
              )}
            </div>
          </div>
        )}
        ...
        <AnimatePresence>
          {qrModal.isOpen && (
            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-8 bg-black/95 backdrop-blur-3xl">
              <div className="bg-[#0a0a0a] border border-white/10 p-20 rounded-[5rem] text-center max-w-sm w-full relative shadow-2xl">
                <button onClick={() => setQrModal(p => ({ ...p, isOpen: false }))} className="absolute top-12 right-12 text-gray-800 hover:text-white p-2 glass rounded-full"><X size={32}/></button>
                <div className="bg-white p-10 rounded-[3.5rem] aspect-square flex items-center justify-center mb-12 mx-auto max-w-[340px] overflow-hidden shadow-inner">
                   {qrModal.connected ? (
                     <div className="flex flex-col items-center"><CheckCircle2 size={120} className="text-green-500 mb-4" /><span className="text-black font-black uppercase text-[12px] tracking-[0.4em]">Handshake Sucesso!</span></div>
                   ) : qrModal.code ? (
                      <img src={qrModal.code} className="w-full h-full object-contain scale-110" alt="QR" />
                    ) : (
                      <div className="flex flex-col items-center gap-4"><Loader2 className="animate-spin text-orange-500" size={60} /><span className="text-black text-[8px] font-black uppercase tracking-widest">Provisionando...</span></div>
                    )}
                </div>
                ...
                <div className="mt-12 flex items-center gap-4 justify-center"><div className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-ping" /><div className="text-[8px] font-black text-gray-800 uppercase tracking-widest">Não feche esta janela durante a leitura</div></div>
              </div>
            </div>
          )}
        </AnimatePresence>
